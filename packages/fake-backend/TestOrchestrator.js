...
class TestOrchestrator {
    ...
    async start() {
        ...
        await this.client.authPaymaster(process.env.PAYMASTER_API_KEY || 'YOUR_API_KEY');

        this.scheduler.start();
        console.log(`Test started. Will send ${this.totalTransactions} transactions over 24 hours.`);
    }
    ...
    async runTransaction() {
        if (!this.scheduler.isRunning || this.transactionsSent >= this.totalTransactions) {
            this.stop();
            return;
        }
        ...
        const selectedNetwork = availableNetworks[Math.floor(Math.random() * availableNetworks.length)]; // Randomly select a network
        ...
        console.log(`Running tx #${this.transactionsSent + 1}: User=${userType}, Type=${txTypeSelected}, Sponsored=${isSponsored}, Network=${selectedNetwork}`);
        ...
        try {
            ...
            const transactionClient = new AAStarClient({ networkName: selectedNetwork });
            const transactionHandler = new TransactionTypeHandler(transactionClient, user.account);

            let txPromise;
            if (txTypeSelected === 'ERC20') {
                txPromise = transactionHandler.executeERC20Transfer('0xYourERC20TokenAddress', '0xRecipientAddress', 1n, { withPaymaster: isSponsored });
            } else if (txTypeSelected === 'NFT') {
                txPromise = transactionHandler.executeNFTMint('0xYourNFTAddress', await user.account.getAccountAddress(), { withPaymaster: isSponsored });
            } else { // DApp
                txPromise = transactionHandler.executeDAppInteraction('0xYourDAppAddress', '0x12345678', { withPaymaster: isSponsored });
            }
            ...
            status = 'FAILED';
            error = e.message;
            console.error(`Transaction failed for ${userType} on ${selectedNetwork}:`, e);
        }
