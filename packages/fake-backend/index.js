...
// const TestOrchestrator = require('./TestOrchestrator'); // <-- DISABLED
...
app.get("/", (req, res) => {
  console.log("--- API: / hit ---");
  res.send(
    "Welcome to the Fake Backend API for CoinJar Demo. This API provides endpoints for user registration, transaction preparation, and broadcasting.",
  );
});
...
app.post("/register/finish", (req, res) => {
  ...
  console.log(
    `[API] Received attestation object:`,
...
app.post("/email/verify-code", (req, res) => {
  ...
  console.log(
    `[API] Received /email/verify-code for email: ${email} with code: ${code}`,
  );
...
app.get("/prices", (req, res) => {
  ...
  const prices = {
    USDT: { USD: 1.0, CNY: 7.25, THB: 36.7 },
    USDC: { USD: 1.01, CNY: 7.32, THB: 37.0 },
...
// --- Test Orchestration Endpoints (DISABLED) ---
/*
